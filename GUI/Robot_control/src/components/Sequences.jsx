...
export default function Sequences({ joints, opening, setShowSequences, setLogs }) {
    ...
    const addPosition = () => {
        ...
        const newPos = {
            joints: { ...joints },
            gripperOpening: opening
        };
        setLogs(prev => [...prev, { time: new Date().toLocaleTimeString(), type: "INFO", values: `Posición añadida: ${[...positions, newPos['gripperOpening']]}` }])
    ...
    return (
        <div className='w-[max(330px,25vw)] rounded-lg text-white bg-[#1F1F1F] border-1 border-solid border-[#4A4A4A]'>
            <div className='w-full rounded-t-lg bg-[#2B2B2B] py-2 px-5 font-bold text-xl items-center flex flex-row'>
                <button onClick={() => setShowSequences(false)} className='px-2 cursor-pointer'>
                ...
                <div className='flex justify-between items-center w-full'>
                    <Select.Root value={value} onValueChange={setValue}>
                        ...
                        <Select.Content className="bg-[#2B2B2B] border border-gray-400 rounded mt-1">
                            {sequences.length > 0 ? sequences.map(pos => (
